...
using Sinodom.ElevatorCloud.Tests.TestDatas;
...
namespace Sinodom.ElevatorCloud.Tests
{
    ...
    /// Provides methods to easily work with <see cref="ElevatorCloudDbContext"/>.
    /// </summary>
    public abstract class AppTestBase : AbpIntegratedTestBase<ElevatorCloudTestModule>
    {
        ...
        {
            void NormalizeDbContext(ElevatorCloudDbContext context)
            {
            ...
            UsingDbContext(context =>
            {
                ...
                new TestDataBuilder(context, 1, 2, 3).Create();
            });
        }
        ...
        protected void UsingDbContext(Action<ElevatorCloudDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<ElevatorCloudDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<ElevatorCloudDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<ElevatorCloudDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<ElevatorCloudDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ElevatorCloudDbContext>())
                {
                    action(context);
                    context.SaveChanges();
                }
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<ElevatorCloudDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ElevatorCloudDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<ElevatorCloudDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<ElevatorCloudDbContext, Task<T>> func)
        {
            T result;
