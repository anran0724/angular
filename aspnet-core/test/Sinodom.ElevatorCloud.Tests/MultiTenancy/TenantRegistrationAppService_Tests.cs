...
namespace Sinodom.ElevatorCloud.Tests.MultiTenancy
{
    public class TenantRegistrationAppService_Tests : AppTestBase
    {
        ...
        [MultiTenantFact]
        public async Task EccpCompanyRegistration()
        {
            // Arrange
            ...
            //Act
            var registerResult = await _tenantRegistrationAppService.RegisterTenant(
                                     new RegisterTenantInput
                                         {
                                             EditionId = 2,
                                             AdminEmailAddress = "admin@volosoft.com",
                                             AdminPassword = "123qwe",
                                             ...
                                             AptitudePhotoIdFileToken = "4811d3f3-0bfa-4672-b875-2d47299d175d.jpg",
                                             BusinessLicenseId = businessLicenseId,
                                             BusinessLicenseIdFileToken = "4811d3f3-0bfa-4672-b875-2d47299d175d.jpg",
                                             IsMember = false,
                                             LegalPerson = "企业法人",
                                             Mobile = "11123123121"
                                         });
            ...
            await UsingDbContextAsync(async context =>
                {
                    ...
                    tenant.SubscriptionEndDateUtc.HasValue.ShouldBe(false);
                    tenant.TenancyName.ShouldBe("Volosoft");


                    var company = await context.ECCPBaseMaintenanceCompanies.FirstOrDefaultAsync(t => t.TenantId == registerResult.TenantId);
                    company.ShouldNotBeNull();

                    var companyExtension = await context.EccpMaintenanceCompanyExtensions.FirstOrDefaultAsync(t => t.MaintenanceCompanyId == company.Id);
                    companyExtension.ShouldNotBeNull();
                    companyExtension.AptitudePhotoId.ShouldNotBeNull();
                });
        }
    }
}
