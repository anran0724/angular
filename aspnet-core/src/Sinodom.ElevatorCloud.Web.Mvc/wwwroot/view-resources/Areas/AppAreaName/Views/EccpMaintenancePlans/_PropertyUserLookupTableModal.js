(function ($) {
    app.modals.PropertyUserLookupTableModal = function () {

        var _modalManager;

        var _eccpMaintenancePlansService = abp.services.app.eccpMaintenancePlans;
        var _$propertyUserTable = $('#PropertyUserTable');
        ...
        var bindData = function () {
            if (_maintenancePlan.propertyUserIds != "") {
                a = _maintenancePlan.propertyUserIds.split(',');
                var propertyUserNames = _maintenancePlan.propertyUserNames.split(',');
                $.each(propertyUserNames, function (index, value) {
        ...
        var dataTable = _$propertyUserTable.DataTable({
            paging: true,
            serverSide: true,
            processing: true,
            listAction: {
                ajaxFunction: _eccpMaintenancePlansService.getAllPropertyUserForLookupTable,
                inputFilter: function () {
                    return {
                        filter: $('#PropertyUserTableFilter').val()
                    };
                }
            },
            columnDefs: [
        ...
        $('#PropertyUserTable tbody').on('click', '[id*=selectcheck]', function () {
            var data = dataTable.row($(this).parents('tr')).data();
            if ($(this).is(':checked')) {
                a.push($(this).val());
                _selecthtml += "<div class='btn-group m-btn-group' role='group' aria-label='...' id='" + data.id + "'><button type='button' name='displayName' class='btn btn-brand'>" + data.displayName + "</button><button type='button' class='btn btn-success' id='removebtn' aria-label='Close'><span aria-hidden='true'>X</span></button></div>";
        ...
        $('#PropertyUserTable tbody').on('click', '[id*=selectbtn]', function () {
            var data = dataTable.row($(this).parents('tr')).data();
            _modalManager.setResult(data);
            _modalManager.close();
        });
        ...
        $('#GetPropertyUserButton').click(function (e) {
            e.preventDefault();
            getPropertyUser();
        });
