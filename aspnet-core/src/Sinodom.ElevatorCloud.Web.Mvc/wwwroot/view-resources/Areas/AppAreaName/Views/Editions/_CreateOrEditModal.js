(function () {
    ...
    app.modals.CreateOrEditEditionModal = function () {
        ...
        var _eccpEditionsTypeLookupTableModal = new app.ModalManager({
            viewUrl: abp.appPath + 'AppAreaName/Editions/ECCPEditionTypeLookupTableModal',
            scriptUrl: abp.appPath + 'view-resources/Areas/AppAreaName/Views/Editions/_ECCPEditionTypeLookupTableModal.js',
            modalClass: 'ECCPEditionTypeLookupTableModal'
        });
        ...
        $('#OpenECCPEditionTypeLookupTableButton').click(function () {


            console.log(222);
            var edition = $editionInformationForm.serializeFormToObject();

            _eccpEditionsTypeLookupTableModal.open({ id: edition.eccpEditionsTypeId, displayName: edition.editionTypeName }, function (data) {
                $editionInformationForm.find('input[name=editionTypeName]').val(data.displayName);
                $editionInformationForm.find('input[name=eccpEditionsTypeId]').val(data.id);
            });
        });

        $('#ClearECCPEditionTypeNameButton').click(function () {
            $editionInformationForm.find('input[name=editionTypeName]').val('');
            $editionInformationForm.find('input[name=eccpEditionsTypeId]').val('');
        });
