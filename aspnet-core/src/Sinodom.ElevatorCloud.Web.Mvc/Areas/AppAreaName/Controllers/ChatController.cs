...
namespace Sinodom.ElevatorCloud.Web.Areas.AppAreaName.Controllers
{
    ...
    public class ChatController : ChatControllerBase
    {
        ...
        public async Task<ActionResult> GetUploadedObject(string fileId, string contentType)
        {
            var fileObject = await BinaryObjectManager.GetOrNullAsync(Guid.Parse(fileId));
            if (fileObject == null)
            {
                return StatusCode((int)HttpStatusCode.NotFound);
            }
